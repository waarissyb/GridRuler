function addGridRuler () {
	var html = '<div id="grid-ruler" class="mira-p">' +
	...
	'	</div>' +
	'</div>' +
	'<style>#grid-ruler{width:70px;height:60px;font-family:arial;font-size:0;position:fixed;top:0;left:0;z-index:9001;cursor:move;padding:10px 20px 40px;border-radius:10px;border:2px solid transparent;-webkit-transition:background .05s ease-in-out;-moz-transition:background .05s ease-in-out;transition:background .05s ease-in-out;-webkit-user-select:none;-moz-user-select:none}#grid-ruler,#grid-ruler *{box-sizing:content-box;-moz-box-sizing:content-box;-webkit-box-sizing:content-box}#grid-ruler div{position:absolute;left:50%}#grid-ruler #grid-ruler-1{width:10px;height:10px;bottom:80px;margin-left:-30px}#grid-ruler #grid-ruler-2{width:40px;height:40px;top:30px;margin-left:-20px}#grid-ruler #grid-ruler-3{width:20px;height:20px;margin-left:20px}#grid-ruler:hover .controls{opacity:1}#grid-ruler .controls{opacity:.25;background:rgba(100,100,100,.1);height:26px;position:absolute;width:100%;bottom:0;left:0;border-radius:13px;-webkit-transition:opacity .1s ease-in-out}#grid-ruler .controls a{width:26px;height:26px;display:block;position:absolute;color:#000;font-size:16px;line-height:26px;text-decoration:none;cursor:pointer;text-align:center}#grid-ruler .controls a:active{color:#31571b}#grid-ruler .color-toggle{left:0}#grid-ruler .set-size{margin-left:-12px;left:50%}#grid-ruler .keylink{right:0}#grid-ruler.dragged{border:2px dashed rgba(100,100,100,.5)}#grid-ruler.mira-p #grid-ruler-1{background-color:#db1d4d}#grid-ruler.mira-p #grid-ruler-2{background-color:#aac02c}#grid-ruler.mira-p #grid-ruler-3{background-color:#31571b}#grid-ruler.mira-t #grid-ruler-1{background-color:#f37621}#grid-ruler.mira-t #grid-ruler-2{background-color:#ebb927}#grid-ruler.mira-t #grid-ruler-3{background-color:#8c6239}</style>';
...
//		console.log(this.parentNode.parentNode.className);
	...
	askSize: function (e) {
		...
		for (n = 0; n < 3; n++) {
			dragDrop.squares[n] = prompt('Fill in a number in pixels (without a suffix)', '10');
			while (isNaN(dragDrop.squares[n]) || dragDrop.squares[n] < 5 || dragDrop.squares[n] > 250) {
				dragDrop.squares[n] = prompt('Fill in a number between 5 and 250', '10');
			}
	
			dragDrop.squares[n] = parseInt(dragDrop.squares[n], 10);
	
//			console.log(document.getElementById('grid-ruler-'+ (n+1)));
	...
	setSize: function (e) {	
		...
		document.getElementById('grid-ruler').style.width = (dragDrop.squares[0] + dragDrop.squares[1] + dragDrop.squares[2]) + 'px';
		document.getElementById('grid-ruler').style.height = (dragDrop.squares[1] + dragDrop.squares[2]) + 'px';
	},
...
//	'';
	addGridRuler();
	dragDrop.initElement(document.getElementById('grid-ruler'));
}
